...
You can customize the launch of the app with a few arguments. If you provide a file, like `./server.py path/to/save/todo.json`, it will save the the data in that custom location. This is useful for having multiple TODO lists. If not provided, the app saves the data in the app data directory (e.g., on Linux, `~/.local/share/todo/todo.json`), which is useful for having a global TODO list.

You can also provide a custom port with `--port` (default: 8000) and custom host with `--host` (default: localhost).

# Installation on Linux
You can launch the app immediately by calling `./server.py`. However, we recommend also adding the app to your PATH so that you can more easily open a TODO list from any directory. Here is how I recommend installing this app on Linux:
...
You should now be able to launch the TODO app by calling `todo` in any directory.
